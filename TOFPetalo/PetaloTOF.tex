...
Both processes are very fast (of the order of the ps) and therefore scintillation in LXe does not have a physical raise constant. The two decay constants are related with the decay of the two lowest excited molecular states ($1^{\Eta_u^+},$3^{\Eta_u^+}$). The decay of the singlet state 
($1^{\Eta_u^+}) occurs with a lifetime $\tau_1 =2.2$~ ns, while the lifetime of the triplet state ($3^{\Eta_u^+}) is $\tau_2 =27$~ns.

The relative contribution of SE and SR processes to the LXe scintillation light can be written as\cite{Kubota79}:
\begin{equation}
I = 0.3 I_S + 0.7 I_R
\end{equation}
%
...
After the measurements of \cite{Kubota79}, $I_S$~and $I_R$ can be written as:
\begin{eqnarray}
...
\begin{figure}[!bhtp]
	\centering
	\includegraphics[scale=3]{../img/LXeScintillation2.png}
...
$20 \times 20 \times 12 {\rm mm^3)$). Instead, a conventional segmented LYSO crystal, 2 cm
thick would have a DOI r.m.s. resolution of $20 {\rm mm}/sqrt(12) = 5.8$~mm, thus 13 mm FWHM, resulting in a $\Delta t = 35.2$~ps FWHM.

\begin{figure}[!bhtp]
	\centering
	\includegraphics[scale=3]{../img/DTOFLXeLYSO.png}
...
On the other hand, we find that using blue sensitive SiPMs coated with TPB results in a CRT of about 100 ps FWHM for a PDE of 50\%. This is still an superb result, in particular given the competitive cost of today's SiPMs and  front-end electronics.  It appears, therefore, possible to offer a PET scanner combining the advantages of large sensitivity (due to increased large axial acceptance afforded by the low cost of xenon compared with scintillators like LYSO and the possibility of using larger SiPMs due to the virtual absence of dark current), good energy and spatial resolution, and excellent CRT.     
